...
{-| Show a JSON tree.
-}
view : Node -> Config msg -> State -> Maybe String -> Html msg
view node config state query =
    div
        (styleList css.root)
        (hoverStyles config :: viewNodeInternal 0 config node state query)
...
viewNodeInternal : Int -> Config msg -> Node -> State -> Maybe String -> List (Html msg)
viewNodeInternal depth config node state query =
    let
        boolToString bool =
    ...
    case node.value of
        TString str ->
            viewScalar (css.string colors) ("\"" ++ str ++ "\"") node config query

        TFloat x ->
            viewScalar (css.number colors) (String.fromFloat x) node config query

        TBool bool ->
            viewScalar (css.bool colors) (boolToString bool) node config query

        TNull ->
            viewScalar (css.null colors) "null" node config query

        TList nodes ->
            viewArray depth nodes node.keyPath config state query

        TDict dict ->
            viewDict depth dict node.keyPath config state query
...
viewArray : Int -> List Node -> KeyPath -> Config msg -> State -> Maybe String -> List (Html msg)
viewArray depth nodes keyPath config state query =
    let
        innerContent =
        ...
        viewListItem node =
            li
                (styleList css.li)
                (List.append (viewNodeInternal (depth + 1) config node state query) [ text "," ])
    in
    [ text "[" ] ++ innerContent ++ [ text "]" ]


viewDict : Int -> Dict String Node -> KeyPath -> Config msg -> State -> Maybe String -> List (Html msg)
viewDict depth dict keyPath config state query =
    let
        innerContent =
